...
bash ./get_ref_genome.sh           # /!\ the settings need to be adjusted manually!!
...
python ./get_oligos.py DNA/RNA [applyGCfilter False] [extfolder] # RNA assumes already existing transcript sequences. Default> DNA
...
bash ./run_nHUSH_excl.sh -d DNA -L 40 -l 21 -m 3 -t 40 -i 14 # 40 threads for max perf.

# only in case HUSH didn't finish successfully
bash ./unfinished_HUSH.sh

# 7. only if using sublength:
python ./reform_hush_combined.py DNA|RNA|-RNA length sublength until

# 8. Calculate secondary structures and melting temperature
bash ./melt_secs_parallel.sh DNA|RNA
...
python ./cycling_query.py -s DNA -L 40 -m 8 -c 100 -t 40 -g 500 -stepdown 50 -greedy -excl
